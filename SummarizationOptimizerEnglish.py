...
class Preprocessing:
    ...
    def getStopWordList(self):
      ...
      # Additional custom stopwords (if needed)
      more_stopword = [
          'AT_USER', 'URL', 'bgmn', 'tmn2', 'dr', 'pt', 'dg', 'prn', 'bn', 'sbb', 
          'tdk', 'krn', 'ga', 'tak', 'gak', 'gk', 'bkn', 'kan', 'la', 'so', 'dgn',
      ]

      # Combine nltk stopwords with custom stopwords
      data = stop_words + more_stopword

      return data

    def stemming_text(self, sentence):
      """Stemming text by applying stemming to each word in the sentence"""
      # Split sentence into words, apply stemming, and rejoin them
...
class FeatureExtraction(Preparation_Data):
  ...
  def Fitur7(self, df_clean):
    """Detect numerical features in English"""
    # Define English numerical words (you can extend this list as needed)
    english_numbers = [
        "zero", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine", 
        "ten", "eleven", "twelve", "thirteen", "fourteen", "fifteen", "sixteen", 
        "seventeen", "eighteen", "nineteen", "twenty", "first", "second", "third", 
        "fourth", "fifth", "sixth", "seventh", "eighth", "ninth", "tenth", "hundred", 
    ...
    for i in range(len(df_clean)):
        ...
        # Count occurrences of numerical words in the text
        for j in kal.split(" "):
            if j.lower() in english_numbers:
                num += 1

        # Calculate the proportion of numerical words in the text
        fitur7.append(num / len(kal.split(" ")))

    # Add the numerical feature to the DataFrame
    ...
    if self.sin_conv:
        ...
        for j in range(len(self.df_fiks)):
            ...
            for k in kal.split(" "):
                ...
                if synsets:
                    # Replace with the first synonym (lemma name)
                    k = synsets[0].lemmas()[0].name()
                newKal.append(k)

            Sinonim1.append(" ".join(newKal))

        # Add the transformed text to the DataFrame
        self.df_fiks['Sinonim1'] = np.array(Sinonim1)
    else:
        self.df_fiks['Sinonim1'] = self.df_fiks.cleanTeks
...
class Bat_Optimizer:
  ...
  def fit(self):
    for epoch in range(self.epoch):
      ...
      Algorithm     = BatAlgorithm(self.D, self.NP, (epoch*10+10),minimum, maximum, 0.0, self.tuna, (self.tuna*-10), (self.tuna*10),self.Fun)
